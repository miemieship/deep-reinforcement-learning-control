...
x_ =np.linspace(1,36,36)    
temp_reset = np.ones((36, ))*20 
delta_time = 1
...
# 训练一个episode
def run_episode(env, agent0, agent1, agent2, rpm0, rpm1, rpm2):
    ...
    while True:
        step += 1
        action0 = agent0.sample(obs0)  # 采样动作，所有动作都有概率被尝试到
        action1 = agent1.sample(obs1)
        action2 = agent2.sample(obs2)
        action = [action0, action1, action2]
        next_obs, reward, done, _ = env.step(action)
        rpm0.append((obs0, action0, reward, next_obs[0], done))
        rpm1.append((obs1, action1, reward, next_obs[1], done))
        rpm2.append((obs2, action2, reward, next_obs[2], done))
        # train model
        if (len(rpm0) > MEMORY_WARMUP_SIZE) and (step % LEARN_FREQ == 0):
            (batch_obs, batch_action, batch_reward, batch_next_obs,
             batch_done) = rpm0.sample(BATCH_SIZE)
            train_loss0 = agent0.learn(batch_obs, batch_action, batch_reward,
                                     batch_next_obs,
                                     batch_done)  # s,a,r,s',done
        if (len(rpm1) > MEMORY_WARMUP_SIZE) and (step % LEARN_FREQ == 0):
            (batch_obs, batch_action, batch_reward, batch_next_obs,
             batch_done) = rpm1.sample(BATCH_SIZE)
            train_loss1 = agent1.learn(batch_obs, batch_action, batch_reward,
                                     batch_next_obs,
                                     batch_done)  # s,a,r,s',done
        if (len(rpm2) > MEMORY_WARMUP_SIZE) and (step % LEARN_FREQ == 0):
            (batch_obs, batch_action, batch_reward, batch_next_obs,
             batch_done) = rpm2.sample(BATCH_SIZE)
            train_loss2 = agent2.learn(batch_obs, batch_action, batch_reward,
                                     batch_next_obs,
                                     batch_done)  # s,a,r,s',done    

        total_reward += reward
        obs0, obs1, obs2 = next_obs
        if done:
            break
    return total_reward

# 评估 agent, 跑 1 个episode，总reward求平均
def evaluate(env, agent0, agent1, agent2, render=False):
    ...
    for i in range(1):
        ...
        episode_reward = 0
        while True:
            action0 = agent0.predict(obs0)  # 预测动作，只选最优动作
...
class myenv():
    ...
    def __init__(self):
        ...
        self.state_space = spaces.Box(low=20, high=500, shape=(36, ), dtype=np.float32)
        ...
        self.state = None
        self.temp_reset = None
        self.counts = 0

    def get_state(self, P1, P2, P3): # 实验台用
        '''输出控制灯管的功率'''
        out_power = [P1, P2, P3]
    ...
    def step(self, action, count=[0], xs = [0, 0], ys = [1, 1]):
        ...
        #系统当前状态
        self.voltage0 = self.voltage0 + self.actions[action[0]]
        self.voltage1 = self.voltage1 + self.actions[action[1]]
        self.voltage2 = self.voltage2 + self.actions[action[2]]
        if self.voltage0 < 80:
            self.voltage0 = 80
        if self.voltage1 < 80:
            self.voltage1 = 80  
        ...
        #状态转移
        self.next_state = self.get_state(self.voltage0, self.voltage1, self.voltage2)
        next_state = self.next_state
        
        print("action0:{}, voltage0:{} ".format(self.actions[action[0]], self.voltage0))
        print("action1:{}, voltage1:{} ".format(self.actions[action[1]], self.voltage1))
        print("action2:{}, voltage2:{} ".format(self.actions[action[2]], self.voltage2))
        print('state: ',state) 
        ...
        self.temp_reset = np.ones((36, ))*20  # 初始状态保存，防止被更改
...
# 加载模型
# save_path = './project2_1_v1.ckpt'
# agent1.restore(save_path)
# save_path = './project2_2_v1.ckpt'
# agent2.restore(save_path)
# save_path = './project2_3_v1.ckpt'
# agent3.restore(save_path)

# 先往经验池里存一些数据，避免最开始训练的时候样本丰富度不够
while len(rpm) < MEMORY_WARMUP_SIZE:
    run_episode(env, agent1, agent2, agent3, rpm)
print('===========存储完毕============')
max_episode = 1000
...
while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
    ...
    for i in range(0, 50):
        print('$$$$$$$$$$$ {} $$$$$$$$$$$'.format(i))
        total_reward = run_episode(env, agent1, agent2, agent3, rpm)
        episode += 1
        save_path = './project2_v1.ckpt'
        agent1.save(save_path)
        agent2.save(save_path)
        agent3.save(save_path)
        logger.info('episode: {} total_reward: {}'.format(episode, total_reward))
    # test part
    eval_reward = evaluate(env, agent1, agent2, agent3, render=True)  # render=True 查看显示效果
    logger.info('episode:{}    e1_greed:{} e2_greed:{} e3_greed:{} test_reward:{}'.format(
        episode, agent1.e_greed,  agent2.e_greed,  agent3.e_greed, eval_reward))
