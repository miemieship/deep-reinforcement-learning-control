...
# Agent负责算法与环境的交互，在交互过程中把生成的数据提供给Algorithm来更新模型(Model)
# 继承parl.Agent这个类来实现自己的Agent
class Agent(parl.Agent):
    # 构造函数，传入algorithm以及相关的环境参数
    # 此处必须初始化父类 super(Agent, self).__init__(algorithm)
    def __init__(self,
                 algorithm,
                 obs_dim,
                 act_dim,
    ...
    # 定义paddle里面的program，两个program：一个训练，一个预测
